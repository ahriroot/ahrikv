...
const WAL_MAGIC: &[u8; 4] = b"AKVW";
const WAL_VERSION: u32 = 1;

#[derive(Debug, Clone, Decode, Encode)]
pub enum WalEntry {
    Set { db: String, key: String, entry: Entry },
    Delete { db: String, key: String },
    Expire { db: String, key: String, expires_at: u64 },
}
...
impl WalManager {
    pub async fn new(config: PersistenceConfig) -> Result<Self, Box<dyn Error>> {
        ...
        let wal_dir = Path::new(&home_dir).join(".ahriknow/ahrikv/wal");
    ...
    async fn create_new_wal(wal_dir: &Path) -> Option<WalFile> {
        let timestamp = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        let filename = format!("wal_{}.akv", timestamp);
    ...
    pub async fn recover(&self) -> Result<Vec<WalEntry>, Box<dyn Error>> {
        ...
        while let Some(entry) = dir.next_entry().await? {
            let path = entry.path();
            if path.extension().map_or(false, |e| e == "akv") {
