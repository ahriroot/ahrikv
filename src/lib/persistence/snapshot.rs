...
#[allow(dead_code)]
const SNAPSHOT_MAGIC: &[u8; 4] = b"AKVS";
#[allow(dead_code)]
const SNAPSHOT_VERSION: u32 = 1;

#[derive(Debug, Clone, Decode, Encode)]
pub struct SnapshotData {
    pub databases: HashMap<String, HashMap<String, Entry>>,
    pub timestamp: u64,
}
...
impl SnapshotManager {
    pub async fn new(config: PersistenceConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let snapshot_dir = Path::new(&home_dir).join(".ahriknow/ahrikv/snapshot");
    ...
    pub async fn create_snapshot(
        ...
        let filename = format!("snapshot_{}.akv", timestamp);
