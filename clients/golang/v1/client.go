...
func (a *Ahrikv) Expire(key string, ttl *uint64) (*command.ResultExpireString, error) {
	...
	res := rs.(*command.ResultExpireString)
	if !res.OK {
		return res, errors.New("failed to set expire time")
	}
	return res, nil
}
...
func (a *Ahrikv) Set(key, value string, ttl ...uint64) (*command.ResultSetString, error) {
	...
	res := rs.(*command.ResultSetString)
	if !res.OK {
		return res, errors.New("failed to set value")
	}
	return res, nil
}
...
func (a *Ahrikv) HashSet(key string, field string, value string, ttl ...uint64) error {
	cmd := command.HashSet{
		DB:     a.database,
		Key:    key,
		Field:  field,
...
func (a *Ahrikv) HashGet(key string, field string) (*command.ResultHashGet, error) {
	...
	return rs.(*command.ResultHashGet), nil
}
...
func (a *Ahrikv) HashDel(key string, field string) (*command.ResultHashDel, error) {
	...
	return rs.(*command.ResultHashDel), nil
}
...
func (a *Ahrikv) HashExists(key string, field string) (*command.ResultHashExists, error) {
	...
	return rs.(*command.ResultHashExists), nil
}
...
func (a *Ahrikv) HashLen(key string) (*command.ResultHashLen, error) {
	...
	return rs.(*command.ResultHashLen), nil
}
...
func (a *Ahrikv) HashFields(key string) (*command.ResultHashFields, error) {
	...
	return rs.(*command.ResultHashFields), nil
}
